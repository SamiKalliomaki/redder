...
use crate::{database::{Dataset, Value}, buf_reader::{BufReader, FileBufReader, BufReaderExt}};
...
async fn read_length<R: BufReader>(reader: &mut R) -> io::Result<Length> {
    ...
    match flag {
        ...
        0b1000_0000 => {
            unimplemented!()
            // return Ok(Length::Normal(((first & 0x3F) as usize) << 16 | reader.read_u16().await? as usize));
        }
        0b1100_0000 => {
...
async fn read_string<R: BufReader>(reader: &mut R) -> io::Result<BytesMut> {
    ...
    match length {
        Length::Normal(len) => {
            Ok(reader.read_bytes(len as usize).await?)
        }
        Length::Special(mode) => {
            let bytes = match mode {
                ...
                _ => return Err(io::Error::new(io::ErrorKind::InvalidData, "Invalid string length mode")),
            };
            Ok(bytes)
        }
    }
...
pub(crate) async fn read_rdb(file: File) -> anyhow::Result<Vec<Dataset>> {
    ...
    anyhow::ensure!(magic.as_ref() == b"REDIS", "Invalid RDB file, invalid magic: {:?}, expected REDIS.", magic);

    let version = reader.read_bytes(4).await?;
    anyhow::ensure!(version.as_ref() == b"0003", "Invalid RDB file, invalid version: {:?}, expected 0003.", version);
    ...
    loop {
        ...
        match opcode {
            ...
            0xFE => {
                ...
                anyhow::ensure!(db == Length::Normal(0), "Only DB 0 is supported right now, got: {:?}", db);
            },
            0xFF => {
                // EOF
                break;
